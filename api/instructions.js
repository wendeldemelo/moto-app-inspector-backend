...
4. Do not mention technical terms like "SDK" or "API Level" to the user, translate it to "optimization version".`;
        ...
        // Extracts the text generated by AI.
        if (geminiData && geminiData.candidates && geminiData.candidates[0].content.parts[0].text) {
            ...
            return response.status(200).json({ 
                instructions: "The AI ​​assistant is temporarily experiencing high request load. As a security measure, please use the system's native shortcuts below to audit the application's configuration." 
            });
        }

    } catch (error) {
        return response.status(500).json({ 
            instructions: "We were unable to connect to the AI ​​engine. We recommend reviewing the permissions or uninstalling the application through the native panels listed below." 
        });
    }
}
